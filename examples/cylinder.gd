extends Node3D
## Cylinder example scene: curated debug cylinder parameter samples.

const DESIGN_WIDTH := 1280
const DESIGN_HEIGHT := 720
const CYLINDER_BASE_Y := 0.1
const CYLINDER_RADIUS := 0.28
const CYLINDER_HALF_HEIGHT := 0.35
const CYLINDER_BASE_LINE_WIDTH := 2.0
const CYLINDER_JOINT_LINE_WIDTH := 8.0
const SAMPLE_SPACING := 0.84
const RADIUS_SAMPLE_SPACING := 1.05
const HALF_HEIGHT_SAMPLE_SPACING := 1.08
const SAMPLE_START_Z := -0.45
const COLUMN_SPACING := 2.0
const LABEL_Y := CYLINDER_BASE_Y + 0.01
const TITLE_Z := 0.35
...
const HALF_HEIGHT_VALUES := [0.225, 0.325, 0.425, 0.525]
...
const ROTATION_TUNNEL_HALF_HEIGHT := 0.41
const ROTATION_TUNNEL_DEPTH_STEP := 0.16
const ROTATION_TUNNEL_SCALE_STEP := 0.05
const ROTATION_TUNNEL_Y := 0.35
const CYLINDER_S_SAMPLE_COUNT := 8
const CYLINDER_S_TUNNEL_RADIUS := 0.34
const CYLINDER_S_TUNNEL_HALF_HEIGHT := 0.41
const CYLINDER_S_TUNNEL_DEPTH_STEP := 0.16
const CYLINDER_S_TUNNEL_SCALE_STEP := 0.05
const CYLINDER_S_TUNNEL_Y := 0.35
const JOINT_VALUES := [
...
func _ready() -> void:
  ...
  _debug_draw.line_width = CYLINDER_BASE_LINE_WIDTH
  _create_labels()
...
func _draw_cylinder_s_samples(column: int) -> void:
  for row in CYLINDER_S_SAMPLE_COUNT:
    var scale := 1.0 - float(row) * CYLINDER_S_TUNNEL_SCALE_STEP
    var hue := fmod(float(row) / float(CYLINDER_S_SAMPLE_COUNT) + 0.6, 1.0)
    var center := _get_cylinder_s_tunnel_center(column, row)
    var rotation := _get_cylinder_s_tunnel_rotation(row)
    var radius := CYLINDER_S_TUNNEL_RADIUS * scale
    var half_height := CYLINDER_S_TUNNEL_HALF_HEIGHT * scale
    var color := Color.from_hsv(hue, 0.7, 1.0, 0.35)
    _debug_draw.draw_cylinder_s(center, rotation, radius, half_height, color)
    _debug_draw.draw_cylinder_n(
      center,
      rotation,
      radius,
...
func _get_column_sample_count(column: int) -> int:
  match column:
    ...
    8:
      return JOINT_VALUES.size()
    9:
      return CYLINDER_S_SAMPLE_COUNT
    _:
      return 0
...
func _get_column_label(column: int) -> String:
  match column:
    ...
    2:
      return "cylinder(half height)"
    3:
      return "cylinder(segments)"
    4:
      return "cylinder(side segments)"
    5:
      return "cylinder(rotation)"
    6:
      return "cylinder(joint)"
    7:
      return "cylinder_n"
    8:
      return "cylinder_n(joint)"
    9:
      return "cylinder_s"
    _:
      return "cylinder"
...
func _get_column_label_x(column: int) -> float:
  ...
  var max_x := -INF
  for row in _get_column_sample_count(column):
    var bounds := _get_cylinder_x_bounds(column, row)
...
func _get_sample_label_position(column: int, row: int) -> Vector3:
  return Vector3(_get_cylinder_x_bounds(column, row).x - SAMPLE_LABEL_GAP, LABEL_Y, _get_sample_label_z(column, row))
...
func _get_sample_label_z(column: int, row: int) -> float:
  ...
  if column == 9:
    return _get_cylinder_s_tunnel_center(column, row).z
...
func _get_cylinder_center(column: int, row: int) -> Vector3:
  var half_height := _get_column_half_height(column, row)
  return Vector3(_get_column_x(column), CYLINDER_BASE_Y + half_height, _get_sample_z(column, row))


func _get_rotation_tunnel_center(column: int, row: int) -> Vector3:
  var center := _get_cylinder_center(column, row)
  center.y += ROTATION_TUNNEL_Y
  center.z -= float(row) * ROTATION_TUNNEL_DEPTH_STEP
  return center


func _get_cylinder_s_tunnel_center(column: int, row: int) -> Vector3:
  var center := _get_cylinder_center(column, row)
  center.y += CYLINDER_S_TUNNEL_Y
  center.z -= float(row) * CYLINDER_S_TUNNEL_DEPTH_STEP
  return center
...
func _get_cylinder_s_tunnel_rotation(row: int) -> Quaternion:
  var basis := Basis.IDENTITY
  basis = basis.rotated(Vector3.RIGHT, -PI * 0.26)
  basis = basis.rotated(Vector3.BACK, float(row) * PI * 0.10)
  basis = basis.rotated(Vector3.UP, float(row) * PI * 0.08)
...
func _get_column_center(column: int, row: int) -> Vector3:
  ...
  if column == 9:
    return _get_cylinder_s_tunnel_center(column, row)
  return _get_cylinder_center(column, row)
...
func _get_column_rotation(column: int, row: int) -> Quaternion:
  ...
  if column == 9:
    return _get_cylinder_s_tunnel_rotation(row)
...
func _get_column_radius(column: int, row: int) -> float:
  ...
  if column == 9:
    return CYLINDER_S_TUNNEL_RADIUS * (1.0 - float(row) * CYLINDER_S_TUNNEL_SCALE_STEP)
...
func _get_column_half_height(column: int, row: int) -> float:
  ...
  if column == 9:
    return CYLINDER_S_TUNNEL_HALF_HEIGHT * (1.0 - float(row) * CYLINDER_S_TUNNEL_SCALE_STEP)
  return CYLINDER_HALF_HEIGHT
...
func _get_cylinder_x_bounds(column: int, row: int) -> Vector2:
  ...
  for i in segment_count:
    ...
    var radial := (basis.x * cos(angle) + basis.z * sin(angle)) * radius
    for point in [bottom_center + radial, top_center + radial]:
      min_x = minf(min_x, point.x)
      max_x = maxf(max_x, point.x)

  for i in side_segment_count:
    var side_angle := TAU * float(i) / float(side_segment_count)
    var side_radial := (basis.x * cos(side_angle) + basis.z * sin(side_angle)) * radius
    for side_point in [bottom_center + side_radial, top_center + side_radial]:
...
func _get_normal_sample_width(row: int) -> float:
  if row == 3:
    return 10.0
  return CYLINDER_BASE_LINE_WIDTH
